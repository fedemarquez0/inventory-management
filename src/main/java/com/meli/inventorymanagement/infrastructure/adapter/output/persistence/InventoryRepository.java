...
public interface InventoryRepository extends R2dbcRepository<Inventory, Long> {
    ...
    @Query("SELECT i.* FROM inventory i " +
           ...
           "WHERE p.sku = :sku")
    Flux<Inventory> findByProductSku(@Param("sku") String sku);

    @Query("SELECT i.* FROM inventory i " +
           "JOIN products p ON i.product_id = p.id " +
           "WHERE p.sku = :sku AND i.store_id = :storeId")
    Mono<Inventory> findByProductSkuAndStoreId(@Param("sku") String sku, @Param("storeId") Long storeId);

    @Query("SELECT i.* FROM inventory i " +
           "WHERE i.product_id = :productId AND i.store_id = :storeId")
    Mono<Inventory> findByProductIdAndStoreId(@Param("productId") Long productId, @Param("storeId") Long storeId);
}
